...
class User:
    ...
    def save_info(self, writer):
        with open("user_info.csv", "w", newline="") as csvfile:
            ...
            writer.writerow(["Name", "Contact Number", "Address", "Temperature", "Destination","Vaccination Status", "symptoms", "History", "Covid-test"])
            writer.writerow([self.__name, self.__contact_no, self.__address, self.__temp, self.__destination, self.__content._Content__vaccination_status, self.__content._Content__symptoms, "", self.__content._Content__test])
